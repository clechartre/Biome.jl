...
The biome concept has deep roots in both ecology and biogeography. 
...
- The dominant PFT
- The subdominant PFT
- The dominant woody type, it might be the dominant PFT
...
You can define any rule base that links PFTs to Biome. The only requirement is a PFT. 

First, you need to define the Biomes themselves. Give them a name, and a numerical value (useful for encoding in output and for plotting later). 
...
In the below example, we assign Savanna when the dominant PFT is a C4, a Tropical Evergreen Forest if PFT is TropicalEvergreen and Temperate Deciduous Forest for Temperate Deciduous. 
