...
function ModelSetup(::Type{M};
    ...
    co2::Float64 = 378.0,
    PFTList = nothing) where {M<:BiomeModel}
    ...
    return ModelSetupObj{M}(M(), lon, lat, co2, rasters, PFTList)
end
    ...
    for y in 1:cnty
        ...
        for x in 1:current_chunk_size
            ...
            process_cell(
                ...
                dz, lon_chunk, output_stack, model, PFTList
            )
        end
...
function process_cell(
    ...
    # Run the model 
    output = run(model, input, PFTList)
