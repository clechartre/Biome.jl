...
@testset "Climate Data Tests" begin
    ...
    @testset "Positive Test - Temperate Climate" begin
        ...
        tas = [-2.0, 1.0, 6.0, 12.0, 18.0, 22.0, 25.0, 23.0, 18.0, 12.0, 6.0, 1.0]
        pr = [50.0, 60.0, 70.0, 80.0, 90.0, 100.0, 80.0, 70.0, 80.0, 90.0, 70.0, 60.0]
        ...
        tcm, gdd5, gdd0, twm = climdata(tas, pr, dtas)
        ...
        expected_gdd5 = sum(max.(dtas .- 5.0, 0.0))
        @test gdd5 ≈ expected_gdd5
        
        # GDD0 should be positive (has days > 0°C)
        @test gdd0 > 0.0
        expected_gdd0 = sum(max.(dtas, 0.0))
        @test gdd0 ≈ expected_gdd0
    end
    
    @testset "Tropical Climate Tests" begin
        # Tropical climate - consistently warm
        tas_tropical = [26.0, 27.0, 28.0, 29.0, 28.0, 27.0, 26.0, 26.0, 27.0, 28.0, 27.0, 26.0]
        pr_tropical = [200.0, 150.0, 100.0, 50.0, 30.0, 20.0, 25.0, 40.0, 80.0, 150.0, 180.0, 220.0]
        dtas_tropical = fill(27.0, 365)  # Simplified constant daily tas
        
        tcm_trop, gdd5_trop, gdd0_trop, twm_trop = climdata(tas_tropical, pr_tropical, dtas_tropical)
        
        @test all(isfinite.([gdd5_trop, tcm_trop, gdd0_trop, twm_trop]))
        
        # TCM should be minimum temperature
        @test tcm_trop == minimum(tas_tropical)
        @test tcm_trop ≈ 26.0
        
        # TWM should be maximum temperature
        @test twm_trop ≈ maximum(tas_tropical)
        @test twm_trop ≈ 29.0
    ...
    @testset "Arctic Climate Tests" begin
        ...
        pr_arctic = [10.0, 8.0, 12.0, 15.0, 20.0, 25.0, 30.0, 28.0, 22.0, 18.0, 12.0, 8.0]
        dtemp_arctic = vcat(
        ...
        tcm_arctic, gdd5_arctic, gdd0_arctic, twm_arctic = climdata(temp_arctic, pr_arctic, dtemp_arctic)
    ...
    @testset "GDD Calculation Tests" begin
        ...
        pr_test = fill(50.0, 12)
        dtemp_all_warm = fill(20.0, 365)
        
        _, gdd5_warm, _, _ = climdata(temp_all_warm, pr_test, dtemp_all_warm)
        ...
        _, gdd5_cold, _, _ = climdata(temp_all_cold, pr_test, dtemp_all_cold)
        @test gdd5_cold ≈ 0.0
        ...
        _, gdd5_mixed, _, _ = climdata(temp_mixed, pr_test, dtemp_mixed)
        ...
        _, gdd5_threshold, _, _ = climdata(temp_threshold, pr_test, dtemp_threshold)
    ...
    @testset "Temperature Extremes Tests" begin
        ...
        pr_test = fill(30.0, 12)
        dtemp_test = fill(10.0, 365)
        
        tcm_test, gdd5_test, gdd0_test, twm_test = climdata(temp_test, pr_test, dtemp_test)
        ...
        tcm_extreme, gdd5_extreme, gdd0_extreme, twm_extreme = climdata(temp_extreme_cold, pr_test, dtemp_extreme_cold)
        ...
        tcm_hot, gdd5_hot, gdd0_hot, twm_hot = climdata(temp_extreme_hot, pr_test, dtemp_extreme_hot)
    ...
    @testset "Type Consistency Tests" begin
        ...
        pr_f32 = Float32.(fill(40.0, 12))
        dtemp_f32 = Float32.(fill(18.0, 365))
        
        tcm_f32, gdd5_f32, gdd0_f32, twm_f32 = climdata(temp_f32, pr_f32, dtemp_f32)
    ...
    @testset "Array Length Validation" begin
        ...
        pr_valid = fill(50.0, 12)
        dtemp_valid = fill(15.0, 365)
        
        # Test with wrong temp array length
        @test_throws BoundsError climdata(fill(15.0, 10), pr_valid, dtemp_valid)
        
        # Test with wrong pr array length
        @test_throws BoundsError climdata(temp_valid, fill(50.0, 10), dtemp_valid)
        
        # Test with wrong dtemp array length
        @test_throws BoundsError climdata(temp_valid, pr_valid, fill(15.0, 300))
    ...
    @testset "Seasonal Pattern Tests" begin
        ...
        pr_nh = [60.0, 55.0, 65.0, 70.0, 80.0, 85.0, 90.0, 85.0, 75.0, 70.0, 65.0, 60.0]
        ...
        tcm_nh, gdd5_nh, gdd0_nh, twm_nh = climdata(temp_nh, pr_nh, dtemp_nh)
    ...
    @testset "Edge Case - All Same Values" begin
        ...
        pr_same = fill(25.0, 12)
        dtemp_same = fill(10.0, 365)
        
        tcm_same, gdd5_same, gdd0_same, twm_same = climdata(temp_same, pr_same, dtemp_same)
    ...
    @testset "Mathematical Properties Tests" begin
        ...
        pr_math = [40.0, 45.0, 50.0, 55.0, 60.0, 65.0, 60.0, 55.0, 50.0, 45.0, 40.0, 35.0]
        dtemp_math = fill(15.0, 365)
        
        tcm_math, gdd5_math, gdd0_math, twm_math = climdata(temp_math, pr_math, dtemp_math)
        ...
        _, gdd5_double, _, _ = climdata(temp_math, pr_math, dtemp_math_double)
    ...
    @testset "Boundary Temperature Tests" begin
        ...
        pr_boundary = fill(29.0, 12)
        
        # Daily temps exactly at 5°C
        dtemp_at_threshold = fill(4.9, 365)
        _, gdd5_at, _, _ = climdata(temp_boundary, pr_boundary, dtemp_at_threshold)
        @test gdd5_at ≈ 0.0
        
        # Daily temps just above 5°C
        dtemp_just_above = fill(5.01, 365)
        _, gdd5_above, _, _ = climdata(temp_boundary, pr_boundary, dtemp_just_above)
        @test gdd5_above > 0.0
        @test gdd5_above ≈ 365 * 0.01
        
        # Daily temps just below 5°C
        dtemp_just_below = fill(4.99, 365)
        _, gdd5_below, _, _ = climdata(temp_boundary, pr_boundary, dtemp_just_below)
        @test gdd5_below ≈ 0.0
    end
    ...
    @testset "Environment Data Override Tests" begin
        ...
        tcm_calc, gdd5_calc, gdd0_calc, twm_calc = climdata(temp_test, pr_test, dtemp_test)
        ...
        tcm_env, gdd5_env, gdd0_env, twm_env = climdata(temp_test, pr_test, dtemp_test, env_complete)
        ...
        tcm_partial, gdd5_partial, gdd0_partial, twm_partial = climdata(temp_test, pr_test, dtemp_test, env_partial)
        ...
        tcm_single, gdd5_single, gdd0_single, twm_single = climdata(temp_test, pr_test, dtemp_test, env_single)
        ...
        tcm_empty, gdd5_empty, gdd0_empty, twm_empty = climdata(temp_test, pr_test, dtemp_test, env_empty)
        ...
        tcm_nothing, gdd5_nothing, gdd0_nothing, twm_nothing = climdata(temp_test, pr_test, dtemp_test, nothing)
    ...
    @testset "Environment Data Type Consistency Tests" begin
        ...
        tcm_f32_env, gdd5_f32_env, gdd0_f32_env, twm_f32_env = climdata(temp_f32, pr_f32, dtemp_f32, env_f32)
    ...
    @testset "Environment Data Validation Tests" begin
        ...
        tcm_baseline, gdd5_baseline, gdd0_baseline, twm_baseline = climdata(temp_test, pr_test, dtemp_test)
        ...
        tcm_irr, gdd5_irr, gdd0_irr, twm_irr = climdata(temp_test, pr_test, dtemp_test, env_irrelevant)
        ...
        tcm_ext, gdd5_ext, gdd0_ext, twm_ext = climdata(temp_test, pr_test, dtemp_test, env_extreme)
