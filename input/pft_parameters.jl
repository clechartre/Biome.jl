...
# OrderedDict to maintain order of PFTs
pft_dict = OrderedDict{Symbol, ComponentArray}()

# Create individual NamedTuples for each set of parameters
for i in eachindex(plant_types)
    ...
    # Merge NamedTuples into a single NamedTuple
    merged_params = merge(main_params, constraints, additional_params)

    # Create ComponentArray with the merged NamedTuple
    pft_dict[plant_types[i]] = ComponentArray(merged_params)
end

# Display the first PFT
pft_dict[:tet]