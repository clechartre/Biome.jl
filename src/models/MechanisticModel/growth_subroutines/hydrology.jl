        ...
        for month in 1:12
            ...
            for _ in 1:days[month]
                ...
                if dtemp[d] <= T(-10.0)
                    ...
                    if wr > T(0.0)
                        r1_1 = root * (w[1] / wr)
                        r1_2 = (T(1.0) - root) * (w[2] / wr)
                    end

                    # Update soil water in layer 1
                    if k[5] != T(0.0)
                        w[1] = w[1] + (
                            dprec[d] + dmelt[d] - perc - evap - r1_1 * aet
                        ) / k[5]
                    else
                        w[1] = T(0.0)
                    end

                    # Update soil water in layer 2
                    if k[6] != T(0.0)
                        w[2] = w[2] + (perc - r1_2 * aet) / k[6]
                    else
                        w[2] = T(0.0)
                    end
                ...
                meanwr[month][1] += wr   / T(days[month])
                meanwr[month][2] += w[1] / T(days[month])
                meanwr[month][3] += w[2] / T(days[month])

                if gc != T(0.0)
                    meangc[month] += gc / T(days[month])
                end
                if fvc != T(0.0)
                    meanfvc[month] += fvc / T(days[month])
                end
                meanaet[month] += aet / T(days[month])
